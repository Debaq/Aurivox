...
// Manejo de ISRs, debounce, sleep y sistema de pips
...
extern const int SAMPLE_RATE;  // Definido en main (16000)
static const int PIP_SAMPLES = (SAMPLE_RATE * PIP_DURATION_MS) / 1000;
...
struct PipSystem {
  ...
  bool in_gap;
  float phase;
} pip_system = {false, 0, 0, 0, 0, 0, false, 0.0};
...
static void handle_gain_buttons() {
  ...
  if (btn_gain_down_flag) {
    ...
    if (current_gain_level > 0) {
      ...
      start_pip_sequence(1);  // 1 pip largo para indicar mínimo
    }
  }
}
